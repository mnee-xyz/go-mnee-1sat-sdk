...
func TestGetSpecificTransactionHistory_Integration(t *testing.T) {
	...
	assertions.NoError(err, "Failed to get config, cannot proceed with history test")
	...
	assertions.NoError(err, "GetSpecificTransactionHistory() should not return an error")
