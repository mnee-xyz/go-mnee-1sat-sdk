...
func TestIsMneeScript_Integration(t *testing.T) {
	...
	assertions.NoError(err, "Failed to get config, cannot proceed")
	assertions.NotNil(config.FeeAddress)

	utxos, err := m.GetUnspentTxos(context.Background(), []string{testAddress})
	assertions.NoError(err, "Failed to get UTXOs")
	...
	assertions.NoError(err, "Failed to parse script from Base64")
	...
	assertions.NoError(err, "IsMneeScript should not return an error")
